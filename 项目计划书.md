...
| 路径 | 职责要点 |
|------|----------|
| `app.py` | 页面主题 CSS、`MODE_DEFAULTS` 三种强度、`FillTask` 与 dict 互转、入库/分析/生成主流程、`_stream_three_line_window` 三行流式窗口、`route_slot` 路由反馈 |
| `config.py` | API Key 解析顺序、默认 Base URL、各模型名与温度、`RETRIEVAL_*`、超时重试、路径常量 |
...
| `core/generator.py` | `_build_chat_request`、弱库/低相似路由、`generate_stream` / `generate`、`content_gen_*` 日志 |
...
| `LARGE_LLM_MODEL` | 主写作模型 | 默认 glm-5 |
...
- `python smoke_test_models.py --offline`：路由逻辑（含低相似触发联网）通过。

### 18.2 手工抽检

1. 空库 + 联网开：`content_gen_route` 中 `native_web_search=True`。  
2. 满库高相似 + 联网开：`native_web_search=False`，模型为主模型。  
3. 满库低相似（可控：临时调高阈值或换不相关 query 资料）+ 联网开：应走联网档。  
4. Word 打开无报错；随机抽 5 个任务核对位置与字数量级。  
5. 侧栏切换知识库后检索内容确实变化。

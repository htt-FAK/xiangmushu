...
**文档版本**：1.2  
**版本 1.2：将联网补料从百炼内置搜索迁移至 Firecrawl 免费托管 MCP，零 API key**  
...
#### 3.2.5 联网补料（Firecrawl 免费托管 MCP）

- **开关**：侧栏「联网补料」映射 session `adv_use_tavily`（历史命名保留），由 `FIRECRAWL_ENABLED` 环境变量控制总开关。
- **触发条件**（需开关为开）：`kb_empty` 或检索结果为空（弱库），**或** 有命中但 `best_similarity_est < RETRIEVAL_WEB_SIMILARITY_THRESHOLD`（默认 0.3，估算 `1 - min(distance)`）。
- **请求形态**：`core/firecrawl_search.py` 通过 sync httpx 向 Firecrawl 托管 MCP 端点 `https://mcp.firecrawl.dev/v2/mcp` 发送 JSON-RPC 2.0 `tools/call` 请求，无需 API key，按 IP 限流（5 search/min、10 scrape/min）。项目锁定 `httpx>=0.27,<0.28`。
- **证据映射**：`core/web_search_agent.py` 中的 `fetch_web_evidence` 调用 `search_web_evidence`，将返回结果映射为 `WebFact` 实例，并通过 `SessionWebEvidenceCache` 按 `task_type|target_chapter|description[:240]` 做会话级缓存。
- **注入方式**：`core/generator.py` 的 `prepare_bundle_from_evidence` 中，当 `use_plus` 分支为真时，将 `WebFact` 列表格式化为 `【联网证据】` 文本块，手动拼接到 `ref_texts` 末尾，再组装进用户提示词。不再设置 `extra_body["enable_search"]`，也不再切换到 `VISION_WEB_MODEL`。
- **生成档位**：联网补料生效时 `generation_tier` 标记为 `"main_writer_web_evidence"`（不再有 `vision_web` 档位），系统提示词切换为 `SYSTEM_PROMPT_WEB_CREATIVE`，指导模型综合知识库与联网证据完成正文。
- **失败模式**：静默跳过。任何错误（超时、HTTP 异常、限流、解析失败）仅记录 warning 日志并返回空列表，不回落到 LLM 内置搜索，生成流程在缺少联网证据的情况下继续执行。

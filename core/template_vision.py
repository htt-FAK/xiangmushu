...
VISION_TEMPLATE_JSON_PROMPT = """你是 Word 排版与申报模板分析专家。下面将提供若干页由 Word 模板渲染成的页面截图（按页序）。
...
def get_or_build_template_vision_profile(
    ...
    *,
    ...
    profile 含 layout_notes / fill_strategy / style_observations / chapter_hints；
    失败时含 error 键，仍可能含 ooxml_fallback。
    """
    cp = _cache_path(docx_path)
    if not force_refresh and os.path.isfile(cp):
        try:
            with open(cp, "r", encoding="utf-8") as f:
                blob = json.load(f)
            prof = blob.get("profile") or {}
            if isinstance(prof, dict):
                return prof, "已使用本地缓存的模板视觉摘要（删除 data/.cache/template_vision 可强制重建）。"
        except Exception as e:
            _LOG.debug("cache read fail: %s", e)
    ...
    if substantial:
        try:
            ...
            _LOG.warning("vision cache write failed: %s", e)
    else:
        status_parts.append(

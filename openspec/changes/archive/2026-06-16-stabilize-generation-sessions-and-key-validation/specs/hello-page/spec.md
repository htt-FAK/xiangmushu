...
The home tab SHALL display a short product description and a numbered recommended workflow that references the existing setup and generation journey in actionable order.

#### Scenario: User reads onboarding on home tab

- **WHEN** the user selects the「首页」tab
- **THEN** the UI shows what the product does and ordered steps covering API setup, knowledge preparation, template preparation, and generation entry
- **AND** the workflow points the user to the relevant product areas for each step
